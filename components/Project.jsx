...
function Project({
  ...
  const HighLightComponent = ({ text }) => (
    <div className="bg-normalBackgroundColor w-fit py-2 px-3 rounded-xl">
      <span className="font-otterco font-light text-[14px] capitalize">
        {text}
      </span>
    </div>
  );
  ...
  return (
    <motion.div
      ...
      className="bg-[white] h-full sm:min-h-[416px] w-full sm:min-w-[807px] rounded-2xl"
    >
      <div className="flex sm:items-center justify-center flex-col-reverse sm:flex-row sm:space-x-5 p-[1rem] sm:p-[2.5rem]">
        <div className="space-y-6 mt-[2rem] sm:mt-0">
          <div>
            <h1 className="font-otterco font-medium leading-[2.5rem] text-[2rem]">
              {title}
            </h1>
          ...
          <div className="space-y-2">
            {React.Children.toArray(
              features.map((feature) => <HighLightComponent text={feature} />)
            )}
          </div>
